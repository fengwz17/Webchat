...
class Article(models.Model):
    ...
    category = models.ForeignKey("Category",verbose_name=u"所属板块", on_delete=models.CASCADE,) #由于Category类在它的下方,所以要引号引起来,Django内部会自动反射去找
    content = models.TextField(verbose_name=u"文章内容")
    head_img = models.ImageField(null=True,blank=True, verbose_name=u"文章标题图片",upload_to="uploads")
...
class Comment(models.Model):
    ...
    parent_comment = models.ForeignKey('self', related_name='my_clildren', blank=True,null=True,verbose_name=u'父评论', on_delete=models.CASCADE,)
    comment_choices = ((1,u"评论"),
                       (2,u"点赞"))
    comment_type = models.IntegerField(choices=comment_choices, default=1, verbose_name=u"评论类型")
    user = models.ForeignKey('UserProfile', verbose_name=u"评论人", on_delete=models.CASCADE,)
...
class UserProfile(models.Model):
    user = models.OneToOneField(User,verbose_name=u"关联Django内部的用户", on_delete=models.CASCADE,)
    ...
    #friends = models.ManyToManyField('self', related_name='my_frends', blank=True)
    def __str__(self):
        return self.name
